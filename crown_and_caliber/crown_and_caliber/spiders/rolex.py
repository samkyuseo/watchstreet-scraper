...
class RolexSpider(scrapy.Spider): 
    ...
    def parse(self, response): 
        for i in range(1, self.page): 
            yield response.follow(f'https://www.crownandcaliber.com/collections/rolex-watches?page={i}', callback=self.parse_page)

    def parse_page(self, response): 
        links = response.css('a.grid-view-item__link::attr(href)').getall()
        # Scrape all watch page links and crawl
        for link in links:
            yield response.follow(f'https://www.crownandcaliber.com{link}', callback=self.parse_watch)
    ...
    def parse_watch(self, response):
        ...
        for ps in prod_specs:
            ...
            if key and value: 
                snake_case_key = key.strip().replace('-', '').strip().replace(' ', '_').lower()
        ...
        yield {
            ...
            'nickname': None if not has_nickname else response.css('span.main-product-name::text').getall()[1].strip(), 
